(function($) {
    var WidgetElements_ViewsHandler = function($scope, $) {
        ...
        if (elementSettings.dce_views_style_format !== "slideshow" && !(elementSettings.dce_views_style_format === "table" && elementSettings.dce_views_style_table_data)) {
            return
        }
        let swiper_class = '.swiper';
        var elementSwiper = $scope.find(swiper_class);
        var speed = elementSettings.transition_speed;
        var disableOnInteraction = Boolean(elementSettings.pause_on_interaction) || !1;
        var loop = !1;
        if (elementSettings.dce_views_style_format === "slideshow") {
            ...
            if (elementSettings.navigation != "none") {
                if (elementSettings.navigation == "both" || elementSettings.navigation == "arrows") {
                    let nextSelector = id_post ? ".elementor-element-" + id_scope + '[data-post-id="' + id_post + '"] .elementor-swiper-button-next' : ".elementor-swiper-button-next";
                    let prevSelector = id_post ? ".elementor-element-" + id_scope + '[data-post-id="' + id_post + '"] .elementor-swiper-button-prev' : ".elementor-swiper-button-prev";
        ...
        if (elementSettings.dce_views_style_format === "table" && elementSettings.dce_views_style_table_data) {
            ...
            if (elementSettings.dce_views_style_table_data_buttons) {
                options.dom = 'Bfrtip';
                options.buttons = ['copyHtml5', 'excelHtml5', 'csvHtml5', 'pdfHtml5']
            }
            if (elementSettings.dce_views_style_table_data_colreorder) {
                options.colReorder = !0
            }
